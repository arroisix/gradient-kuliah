...
const CreditDropdown = (): JSX.Element => {
    ...
    const [mobileStep, setMobileStep] = useState<'summary' | 'select'>(
        'summary'
    );

    const handleMobileSetOpen = (v: boolean) => {
        setIsMobileOpen(v);
        if (!v) setMobileStep('summary');
    };

    const onDesktopTopUpClick = () => {
        setIsPortalOpen(false);
        setIsOpen(true);
    };

    const onTopupClick = () => {
        router.push('/topup?amount=' + selectedAmount);
    };
    ...
    return (
        <>
            ...
            <Modal
                ...
                setOpen={setIsOpen}
                variant="dark"
                className="!max-w-[520px] p-4">
                <CreditPicker
                    credit={isLoading || !data ? undefined : data.credit}
                    isLoading={isLoading}
                ...
                {mobileStep === 'summary' ? (
                    ...
                    <>
                        <div className="px-4 py-6">
                            <CreditPicker
                                credit={
                                    isLoading || !data ? undefined : data.credit
                                }
                                isLoading={isLoading}
                                selectedAmount={selectedAmount}
                                setSelectedAmount={setSelectedAmount}
