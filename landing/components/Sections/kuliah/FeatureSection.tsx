...
const BadgeIcon = ({
    ...
    type: 'play' | 'sparkles' | 'chart';
}): JSX.Element => {
    switch (type) {
        case 'play':
            return (
                <Play className="w-4 h-4 text-[#B6A6F3] fill-[#B6A6F3]" />
            );
        case 'sparkles':
            return <Sparkles className="w-4 h-4 text-[#B6A6F3]" />;
        case 'chart':
            return <BarChart3 className="w-4 h-4 text-[#B6A6F3]" />;
    }
};
    ...
    return (
        <section className="w-full bg-black py-16">
            <div className="max-w-[1280px] mx-auto px-6 md:px-[60px]">
                {/* Section Title */}
                <h2 className="animate-fade-up animate-once animate-duration-500 text-[24px] md:text-[30px] font-bold text-white text-center mb-12">
                    {title}
                </h2>

                {/* Feature Card */}
                <div className="animate-fade-up animate-once animate-duration-500 animate-delay-150 flex justify-center">
                    <div className="group relative w-full max-w-[600px] bg-graphite-800 rounded-[32px] border border-white/5 overflow-hidden transition-all duration-300 hover:border-[#B6A6F3] hover:shadow-[0_0_30px_rgba(182,166,243,0.15)]">
                        {/* Purple glow at bottom */}
                        <div
                            className="absolute bottom-[-241px] left-1/2 -translate-x-1/2 w-[702px] h-[380px] pointer-events-none"
                            style={{
                                background:
                                    'radial-gradient(ellipse at center, rgba(95, 43, 206, 0.4) 0%, transparent 70%)'
                            }}
                        />
                        ...
                        <div className="relative flex flex-col gap-4 items-center pt-8 pb-8 px-8 text-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[#B6A6F3] text-sm font-bold tracking-[0.7px] uppercase">
                                    {data.badge}
                                </span>
                            </div>
                            ...
                            {/* CTA Button */}
                            <button className="flex items-center gap-1 py-2 text-[#B6A6F3] text-sm font-semibold hover:gap-2 transition-all">
                                {data.cta}
